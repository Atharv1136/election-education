...
import { Trophy, RefreshCcw, Share2, ArrowRight } from 'lucide-react'
...
export default function Quiz() {
  ...
  const finishQuiz = async () => {
    ...
    if (user && questions.length > 0) {
      await supabase.from('quiz_scores').insert({
        user_id: user.id,
        score: score + (answers[currentIndex]?.isCorrect ? 1 : 0), // account for last question
        ...
        <div className="max-w-2xl mx-auto">
          <div className="card p-8 sm:p-10 text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-4">
              <button onClick={() => setDifficulty(null)} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-hero-gradient text-white font-semibold hover:opacity-90 transition-opacity shadow-sm">
                <RefreshCcw size={18} /> Retake Quiz
              </button>
              <button 
                onClick={() => navigator.share?.({ title: 'My ElectIQ Score', text: `I scored ${finalScore}/${questions.length} on the ElectIQ ${difficulty} quiz! Are you election ready?` }).catch(()=>{})}
          ...
          <div className="space-y-4">
            {answers.map((ans, i) => (
              <div key={i} className={`card p-5 border-l-4 ${ans.isCorrect ? 'border-l-success bg-green-50/30' : 'border-l-error bg-red-50/30'}`}>
  ...
  if (questions.length === 0) {
    return <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-civic-200 border-t-civic-600 rounded-full animate-spin"/></div>
  }
